 ...
 * 이 파일은 서버 측에서 수행해야 할 비즈니스 로직을 포함하고 있습니다.
 * Node.js, Python, Java 등 서버 환경으로 이 로직을 이관하세요.
 * API Key는 서버 환경 변수에서 안전하게 관리해야 합니다.
 */

const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });

// 1. 이미지 분석 로직 (Controller: /ingredients/analyze)
export const extractIngredientsFromImageLogic = async (base64Image: string): Promise<string[]> => {
  ...
  try {
    ...
    console.error("[Backend Log] Image analysis failed:", error);
    throw new Error("이미지 분석 실패");
  }
};

// 2. 레시피 생성 로직 (Controller: /recipes/generate)
export const generateRecipesLogic = async (ingredients: string[], difficulty: DifficultyLevel): Promise<GeneratedRecipes> => {
  const model = "gemini-2.5-flash";
...
// 3. 요리 이미지 생성 로직 (Controller: /recipes/image)
export const generateRecipeImageLogic = async (recipeName: string, description: string): Promise<string | null> => {
  ...
  try {
    ...
    console.error("[Backend Log] Image generation failed:", error);
    return null;
  }
};